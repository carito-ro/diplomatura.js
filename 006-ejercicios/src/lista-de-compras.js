...
/* `JSON.parse` objeto a string y `JSON.stringify` string a object */
function agregarElto(item) {
    let lista = localStorage.getItem('lista');
    lista = lista.concat(item);
    localStorage.setItem('lista', lista);
}
function eliminarElto(item) {
    let lista = JSON.stringify(localStorage.getItem('lista'));
